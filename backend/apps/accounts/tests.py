...
class AuthFlowTests(APITestCase):
    ...
    def test_refresh_and_logout(self):
        ...
        self.client.post(
            "/api/v1/auth/login/", {"username": "bob", "password": PW}, format="json"
        )
    ...
    def test_logout_revokes_refresh_token_server_side(self):
        ...
        self.client.post(
            "/api/v1/auth/login/", {"username": "dave", "password": PW}, format="json"
        )
...
class UserSearchTests(APITestCase):
    ...
    def test_requires_authentication(self):
        self.assertEqual(self.client.get("/api/v1/auth/users/").status_code, 401)

    def test_prefix_search_returns_matching_usernames(self):
        self.client.force_authenticate(self.user)
        names = [
            u["username"] for u in self.client.get("/api/v1/auth/users/?search=na").data["results"]
        ]
